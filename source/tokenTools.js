...
module.exports.validateTokenSet = async (adapter, tokenSet) => {
    ...
    if (
        ...
        if (
            tokenSet.access_token.length > 0 &&
            ...
            if (await tokenTools.refreshHasExpired(adapter, tokenSet)) {
                throw new Error(
                    'Refresh token has expired. Please use the workflow in adapters admin-ui to generate a new one.',
                );
            }
        }
    } else {
...
module.exports.tokenSetHasExpired = async function (adapter, tokenSet) {
    ...
    if (accessDiff <= 0) {
        adapter.log.warn(
            `Access token has expired on ${new Date(tokenSet.obtained + tokenSet.expires_in * 1000).toLocaleString()} and needs to be refreshed.`,
        );
    } else {
        adapter.log.info(
            `Access token is still valid until ${new Date(tokenSet.obtained + tokenSet.expires_in * 1000).toLocaleString()}`,
        );
    }
    const refreshDiff = new Date(tokenSet.obtained + tokenSet.refresh_expires_in * 1000).getTime() - now;
    if (refreshDiff <= 0) {
        adapter.log.warn(
            `Refresh token has expired on ${new Date(tokenSet.obtained + tokenSet.refresh_expires_in * 1000).toLocaleString()} and needs to be refreshed. Please use the Workflow in Admin-UI to refresh.`,
        );
    } else {
        adapter.log.info(
            `Refresh token is still valid until ${new Date(tokenSet.obtained + tokenSet.refresh_expires_in * 1000).toLocaleString()}`,
        );
    }
...
module.exports.clearTokenStore = async function (adapter) {
    ...
    await adapter
        .getObjectAsync(adapter.namespace)
        .then(async tokenSet => {
            if (tokenSet) {
                tokenSet = tokenTools.getEmptyTokenSet();
                await adapter
                    .extendObject(adapter.namespace, tokenSet)
                    .then(() => {
 ...
 * @returns {Promise<tokenSet>} A valid TokenSet object
 */
module.exports.getTokenSetObj = async function (adapter) {
    // eslint-disable-next-line no-async-promise-executor
    return new Promise(async (resolve, reject) => {
        adapter.log.debug(`Reading adapter namespace: ${adapter.namespace} for tokenSet ...`);
        await adapter
            .getObjectAsync(adapter.namespace)
            .then(async tokenSet => {
                ...
                if (tokenSet) {
                    try {
                        ...
                        if (await tokenTools.tokenSetHasExpired(adapter, tokenSet)) {
                            await tokenTools
                                .refreshTokenSet(adapter, tokenSet)
                                .then(tokenSet => {
                        ...
                        adapter.log.error(`Extracting tokenSet from token store failed.  Native Error: ${error}`);
                    }
                } else {
                    adapter.log.error(`Extracting tokenSet from token store failed.`);
                }
            })
            .catch(async err => {
                //There is no tokenSet. Don't refresh but build a valid tokenSet from scratch if possible
                adapter.log.info(
                    `There is no valid tokenSet in token store. Please run the Authenticate-with-Miele workflow in Admin-UI.`
                );
                adapter.log.debug(`Error getting tokenSet from token store: ${err}`);
            });
    });
};
    ...
    return new Promise((resolve, reject) => {
        ...
        axios(options)
            .then(async result => {
                ...
                await tokenTools.persistTokenSetInTokenStore(adapter, result);
                adapter.log.debug(`NewAuth from server: ${JSON.stringify(result)}`);
                resolve(result);
            })
            .catch(error => {
                if ('status' in error) {
...
module.exports.persistTokenSetInTokenStore = async function (adapter, tokenSet) {
    if (tokenSet.refresh_token.length < 800) {
        adapter.log.warn(
            `Received INVALID refresh_token: ${tokenSet.refresh_token} (Length: ${tokenSet.refresh_token.length}) - aborting persist!`,
        );
        return;
    }
    if (tokenSet.access_token.length < 800) {
        adapter.log.warn(
            `Received INVALID access_token: ${tokenSet.access_token} (Length: ${tokenSet.access_token.length}) - aborting persist!`,
        );
        return;
    }
    ...
    // todo - build and save tokenSet object as subset of native-Object
    tokenSet.obtained_HR = new Date(tokenSet.obtained).toLocaleString();
    await adapter.extendObject(adapter.namespace, { tokenSet: tokenSet });
    /*
    adapter.log.info('Token has changed - reinitializing SSE-connection ...');
    adapter.events['sseErrors'] = 1;
